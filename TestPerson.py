...
# 被测者对象
class Person:
    def __init__(self, inarg, up_bound, down_bound) -> None:
        ...
        self.mode:str = inarg.mode                                  # 模式                          类型：字符串
        self.up_bound:int = up_bound                                # 上边界                        类型：整数
        self.down_bound:int = down_bound                            # 下边界                        类型：整数

    # -----------------获取当前被测者状态（对比线是否位于正常状态）的算法-----------------
    def getstatus(self,points) -> None:
    ...
    # -----------------获取比较线的算法-----------------  可行度（1~5）：3
    def getline(self,frame,points) -> None:
        if len(self.box) > 0:
    ...
    def main(self,points, frame, box) -> None:
        self.box = box
        if len(points) != 0:
            head = points[NOSE][Y]
