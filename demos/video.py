...
if __name__ == '__main__':
    ...
    # video options
    parser.add_argument('-p', '--video-path', default='D:/Code/2p test.mp4', help='path to video file,2p test.mp4')
    parser.add_argument('--yt-id', default='yBZ0Y2t0ceo', help='youtube url id')
    parser.add_argument('--cam', action='store_true', help='use camera')
    parser.add_argument('--cai', default=0, help='camera index')
    ...
    # print('data[scales]:',data['scales'],'data[flips]:',data['flips'])
    ...
    if args.cam:
        dataset = LoadWebcam(args.cai, img_size=imgsz, stride=stride)
    else:
    ...
    for i, (path, img, im0, _) in enumerate(dataset):
        ...
        for j, (bbox, pose) in enumerate(zip(bboxes, poses)):
            ...
            cv2.circle(im0_copy, (int(pose[0, 0]), int(pose[0, 1])), args.kp_size, (123,255,123), args.kp_thick)
            ...
            cv2.circle(im0_copy, (int(pose[9, 0]), int(pose[9, 1])), args.kp_size, (123,255,123), args.kp_thick)
            cv2.circle(im0_copy, (int(pose[10, 0]), int(pose[10, 1])), args.kp_size, (123,255,123), args.kp_thick)
